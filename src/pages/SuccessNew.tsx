...
import { generateLicenseKey, validateLicenseKeyFormat, generateAllDownloadUrls } from "@/lib/license";
  ...
  useEffect(() => {
    ...
    const verifyPaymentFlow = () => {
      ...
      try {
        ...
        const generatedKey = generateLicenseKey(sessionId);
        ...
        if (currentTime - unlocked.timestamp < 60 * 60 * 1000) {
          if (currentTime - unlocked.paymentTimestamp < 48 * 60 * 60 * 1000) {
            ...
            if (unlocked.productKey && validateLicenseKeyFormat(unlocked.productKey).valid) {
              ...
              setProductKey(generateLicenseKey(unlocked.sessionId));
            }
            setIsVerifying(false);
            return;
          }
