...
export const restApiContent: DocumentationSection = {
  ...
  description: 'Complete HTTP API for integrating Local Memory with any application',
...
## 6. System & Management Operations
...
curl -X POST http://localhost:3002/api/v1/analyze \\
  ...
  -d '{
    "analysis_type": "question",
    "question": "What are the advantages of microservices over monoliths?",
...
curl -X POST http://localhost:3002/api/v1/analyze \\
  ...
  -d '{
    ...
    "temporal_timeframe": "quarter"
      ...
      language: 'bash',
      description: 'Advanced AI analysis endpoints for insights'
    },
    {
      id: 'relationship-ops',
...
curl -X POST http://localhost:3002/api/v1/relationships \\
  ...
  -d '{
    ...
    "relationship_type": "expands",
    "strength": 0.9,
    "context": "Kubernetes expands Docker concepts"
  }'

# Map knowledge graph
curl "http://localhost:3002/api/v1/memories/{central-id}/graph?depth=3"`,
      language: 'bash',
      description: 'Building and exploring knowledge relationships via REST API'
    }
  ],
