...
/**
 * Standalone agent install prompt for the /agent-setup page.
 *
 * Unlike getSetupPrompt (used on the post-purchase success page, where the real
 * license key is already known and embedded), this variant is shown before any
 * purchase context exists. It carries a `<YOUR-LICENSE-KEY>` placeholder and a
 * STEP 0 that tells the agent to stop and ask for the key if it wasn't filled
 * in — and, since there's no success page in this context, points manual
 * downloaders at the purchase email instead. The install steps are otherwise
 * identical to getSetupPrompt.
 */
...
OPTION 2 - MANUAL BINARY:
1. Download from the link in my purchase email
2. Create directory: mkdir "C:\\Program Files\\LocalMemory"
3. Move binary: move "C:\\Downloads\\local-memory*.exe" "C:\\Program Files\\LocalMemory\\local-memory.exe"
4. Add to PATH: Add "C:\\Program Files\\LocalMemory" to system PATH
